...
CHAIN_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "robinhood": {
        ...
        "kind": "evm",
        "chain_id": 4663,
        "aliases": {"robinhood", "robinhood_chain", "hood"},
        "enabled": True,
    },
    "base": {
        "label": "Base",
...
VENUE_REGISTRY: dict[str, dict[str, Any]] = {
    "pump_fun": {
        "label": "Pump.fun",
        "chain": "solana",
        "detection": "mint_pattern_plus_launch_dex",
        "enabled": True,
    },
    "pons": {
        "label": "PONS",
        "chain": "robinhood",
        "detection": "factory_contract",
        "enabled": True,
        "factory_addresses": {
            "0xa5aab3f0c6eadf30ef1d3eb997108e976351feb",
            "0x0c37a24f5d23a486fa692d1500881d698b1f77a4",
        },
        "source": "https://docs.ponsfamily.com/",
    },
    "robinhood_chain": {
        "label": "Robinhood Chain generic",
        "chain": "robinhood",
        "detection": "chain_only",
        "enabled": True,
    },
}
...
def normalize_chain(value: str | None) -> str | None:
    ...
    for key, config in CHAIN_REGISTRY.items():
        if needle == key or needle in config.get("aliases", set()):
            return key
    return needle
...
def identify_factory_venue(*, chain: str | None, factory_address: str | None) -> str | None:
    ...
    for venue, config in VENUE_REGISTRY.items():
        if normalize_chain(config.get("chain")) != normalized_chain:
            continue
        if address in config.get("factory_addresses", set()):
            return venue
    return None


def enabled_chains() -> tuple[str, ...]:
    return tuple(key for key, value in CHAIN_REGISTRY.items() if value.get("enabled"))


def enabled_venues() -> tuple[str, ...]:
    return tuple(key for key, value in VENUE_REGISTRY.items() if value.get("enabled"))
